...
PanelObjetos::PanelObjetos(QWidget* parent) : QWidget(parent)
{
    ...
    titulo->setObjectName("PanelTitle");          // <- QSS
    lay->addWidget(titulo);

    m_buscar = new QLineEdit(this);
    m_buscar->setObjectName("SearchBox");         // <- QSS
    ...
    connect(m_buscar, &QLineEdit::textChanged,
            this, &PanelObjetos::filtrar);

    connect(m_listaTablas, &QListWidget::itemDoubleClicked,
            this, [=](QListWidgetItem* it){ emit tablaAbiertaSolicitada(it->text()); });
...
void PanelObjetos::agregarTabla(const QString& nombre)
{
    ...
    it->setSizeHint(QSize(it->sizeHint().width(), 22)); // altura consistente
...
{
    for (int i = 0; i < m_listaTablas->count(); ++i)
        if (m_listaTablas->item(i)->text() == nombre) {
            delete m_listaTablas->takeItem(i);
            return;
        }
}
